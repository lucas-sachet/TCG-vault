...
interface OnboardingWizardProps {
    ...
    onComplete: (
        ...
        selectedLanguages: CardLanguage[],
        selectedGoals: OnboardingGoalKey[],
    ) => void;
}
...
export const OnboardingWizard: React.FC<OnboardingWizardProps> = ({
    ...
    const [selectedLanguages, setSelectedLanguages] = useState<CardLanguage[]>([
        'EN',
    ]);
    const [selectedGoals, setSelectedGoals] = useState<OnboardingGoalKey[]>([
        'collection_tracking',
        'wishlist_management',
    ]);
    ...
    // Toggle helpers
    const handleToggleLanguage = (languageCode: CardLanguage) => {
        setSelectedLanguages(prev => {
            if (prev.includes(languageCode)) {
                if (prev.length === 1) return prev;
                return prev.filter(language => language !== languageCode);
            }
            return [...prev, languageCode];
        });
    };

    const handleToggleGoal = (goalKey: OnboardingGoalKey) => {
    ...
    const handleNext = () => {
        if (currentStep === 4) {
            if (!collectionName.trim()) {
                setInputFeedback(
                    'Please provide a name for your first collection organizer.',
                );
                return;
            }
    ...
    return (
        <div className="min-h-screen bg-[#07090e] text-slate-100 flex flex-col justify-center items-center p-4 relative overflow-hidden select-none">
            ...
            <div className="w-full max-w-xl bg-[#0c0e14] border border-slate-850 rounded-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden z-20 flex flex-col justify-between h-[550px] sm:h-[650px]">
                ...
                <div className="flex justify-between items-center mb-6">
                    <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase font-mono tracking-widest">
                            ONBOARDING SEQUENCE
                        </span>
                    </div>
                            ...
                            {currentStep === 1 && (
                                <div className="space-y-4">
                                    ...
                                    <p className="text-slate-400 text-xs leading-relaxed font-semibold">
                                        PokéVault is an advanced, secure
                                        portfolio manager tailored specifically
                                        for Pokémon TCG collectors and
                                        investors. It enables tracking raw and
                                        graded specimens, monitoring live market
                                        fluctuations, organizing digital
                                        binders, and setting custom high-utility
                                        collection targets.
                                    </p>
                                        ...
                                        <div className="flex gap-2.5 items-start text-xs text-slate-300">
                                            ...
                                            <p className="leading-relaxed">
                                                <span className="font-bold text-slate-200 block mb-0.5">
                                                    Live Valuation Indexing
                                                </span>
                                                Monitor real-time card
                                                valuations tracked against
                                                leading TCG marketplace index
                                                API feeds.
                                            </p>
                                        </div>
                                        <div className="flex gap-2.5 items-start text-xs text-slate-300">
                                            ...
                                            <p className="leading-relaxed">
                                                <span className="font-bold text-slate-200 block mb-0.5">
                                                    Surgical Digital Binders
                                                </span>
                                                Organize your collections inside
                                                interactive tactile binders
                                                complete with grade cert
                                                numbers, conditions, and custom
                                                micro-photos.
                                            </p>
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <h3 className="text-md font-bold text-white uppercase font-mono flex items-center gap-2">
                                            ...
                                            <span>
                                                Select Collecting Preferences
                                            </span>
                                        </h3>
                                        <p className="text-xs text-slate-400 mt-1">
                                            Choose the card language editions
                                            you primarily collect. Allow
                                            multiple selections.
                                        </p>
                                    </div>

                                    <div className="grid grid-cols-1 gap-3 pt-2">
                                        {ONBOARDING_LANGUAGE_OPTIONS.map((languageOption) => (
                                            <div
                                                key={languageOption.code}
                                                onClick={() =>
                                                    handleToggleLanguage(languageOption.code)
                                                }
                                                className={`flex items-start gap-4 p-4 rounded-2xl border transition-all cursor-pointer h-24 ${
                                                    selectedLanguages.includes(languageOption.code)
                                            ...
                                            >
                                                <div
                                                    className={`p-1.5 rounded-xl text-xs font-black shrink-0 ${selectedLanguages.includes(languageOption.code) ? 'bg-[#3B4CCA] text-white' : 'bg-slate-800 text-slate-400'}`}
                                                >
                                                    {selectedLanguages.includes(languageOption.code) ? (
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-400 mt-1 hidden sm:block">
                                            Select your goals to fine-tune
                                            active telemetry and tracking
                                            dashboards.
                                        </p>
                                    </div>

                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                                        {ONBOARDING_GOAL_OPTIONS.map((goalOption) => (
                                            <div
                                                key={goalOption.key}
                                                onClick={() => handleToggleGoal(goalOption.key)}
                                                className={`p-3.5 rounded-xl border transition-all cursor-pointer flex flex-col justify-between h-[105px] ${
                                                    selectedGoals.includes(goalOption.key)
                                                        ? 'bg-[#3B4CCA]/5 border-[#3B4CCA]'
                                                        : 'bg-slate-900/40 border-slate-850 hover:bg-slate-900 hover:border-slate-800'
                                                }`}
                                            >
                                                <div className="flex justify-between items-center">
                                                    <span className="font-bold text-[11px] text-slate-100 block">
                                                        {goalOption.label}
                                                    </span>
                                                    {selectedGoals.includes(goalOption.key) && (
                                                        <Check className="w-3.5 h-3.5 text-[#FFCB05]" />
                                                    )}
                                                </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        <h3 className="text-md font-bold text-white uppercase font-mono flex items-center gap-2">
                                            ...
                                            <span>
                                                Establish First Collection
                                                Binder
                                            </span>
                                        </h3>
                                        <p className="text-xs text-slate-400 mt-1">
                                            Set a custom name for your initial
                                            collection. We will auto-configure a
                                            tactile binder with this name.
                                        </p>
                                    </div>
                                    ...
                                    <div className="space-y-4 pt-2">
                                        ...
                                        <div className="bg-slate-900/30 border border-slate-850/60 p-4 rounded-xl space-y-2">
                                            <span className="text-[8px] bg-[#FFCB05]/10 text-[#FFCB05] font-mono px-2 py-0.5 rounded uppercase font-bold leading-none w-fit block">
                                                AUTOMATIC GENERATION
                                            </span>
                                            <p className="text-[10px] text-slate-400 leading-relaxed">
                                                Creating your primary folder
                                                installs a standard digital
                                                spreadsheet ledger where empty
                                                sleeve grids are prepared to
                                                take physical card registrations
                                                seamlessly once you start.
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-5 text-center py-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-xs text-slate-400 max-w-sm mx-auto font-semibold leading-relaxed">
                                            Your premium PokéVault workspace is
                                            fully configured and secured!
                                            Decryption keys have been assigned
                                            to your profile coordinates.
                                        </p>
                                    </div>
            ...
            <p className="text-[10px] text-slate-500 font-mono tracking-wide uppercase mt-4 text-center z-15">
                PokéVault Security Core &reg; {new Date().getFullYear()} &bull;
                Interactive Sandbox
            </p>
        </div>
    );
